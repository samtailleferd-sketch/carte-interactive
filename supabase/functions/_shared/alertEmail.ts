// Port de src/utils/alertEmailTemplate.js pour l'environnement Deno des Edge
// Functions (les deux runtimes ne partagent pas de bundler commun, donc pas
// d'import cross-arborescence possible) — même rendu, à maintenir en
// parallèle si le gabarit change.
const SITE_URL = "https://samtailleferd-sketch.github.io/carte-interactive/";
const MAX_EQUIPEMENTS = 5;

export interface AlertSalle {
  nom: string;
  ville: string;
  adresse?: string;
  statut: string;
...
export function alertEmailSubject(salle: AlertSalle): string {
  return `Nouvelle salle ajoutée sur Street Map : ${salle.nom} à ${salle.ville}`;
}

export function alertEmailHtml(salle: AlertSalle, unsubscribeUrl: string): string {
